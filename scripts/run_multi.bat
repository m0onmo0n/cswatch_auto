...
if /I not "%~1"=="from_shell" (
  start "Demo2Video — Multi Launcher" /D "%ROOT%" cmd /k "%~f0" from_shell
  exit /b
)

cls
echo ================================================================
echo == Demo2Video multi processor - Application Launcher
echo ================================================================
echo.

:: sanity check
if not exist "%ROOT%d2v_multi" (
  echo [ERROR] 'd2v_multi' not found at: "%ROOT%d2v_multi"
  echo Put this .bat in repo root or scripts\ with the resolver header.
  goto :DONE
)

:: also make sure csdm-fork is visible at repo root (what multi expects)
if not exist "%ROOT%csdm-fork" (
  echo [WARN] '%ROOT%csdm-fork' not found. The multi app may fail to start.
...
:: ---------------------------------------------------------------
:: [2/4] Start multi processor
::     IMPORTANT: run from REPO ROOT so .\csdm-fork resolves
:: ---------------------------------------------------------------
echo [2/4] Starting the multi processor...
set "CSDM_PROJECT_PATH=%ROOT%csdm-fork"
start "D2V Multi - python main.py" /D "%ROOT%" cmd /k call %PYEXE% "d2v_multi\main.py"
